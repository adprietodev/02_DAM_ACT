...
public class AE01_Program extends JFrame {
	...
	private JTextField textWriteDir, textFieldFile01, textFieldFile02, textFieldNewFileName,textMatch;
	private JComboBox comboBoxSelectDir;
	private JButton btnSearch, btnFusion,btnSMatch;
	...
	private static String strNumSizDate,strAscDes;
	...
	public AE01_Program() {
		...
		comboBoxSelectDir.setModel(
				new DefaultComboBoxModel(new String[] { "01_Docs", "02_Facturas", "03_Manuales", "04_Admin" }));
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (directory.exists()) {
					...
					for (int i = 0; i < listTxtFiles.length; i++) {
						filesTxt.add(new File(directory + File.separator + listTxtFiles[i]));
					}
				}

				strAscDes = "Ascendente";
		...
		JLabel lblFusionFile = new JLabel("Fusionar archivos del directorio seleccionado (Escribir nombre solo del TXT)");
		...
		btnFusion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!textFieldFile01.getText().equals("") && !textFieldFile02.getText().equals("")
						&& !textFieldNewFileName.getText().equals("")) {
					...
					File f01 = new File(file01G), f02 = new File(file02G);
					newFile = new File(newFileS);

					if(f01.exists() && f02.exists()) {
						if (newFile.exists()) {
							String res = JOptionPane.showInputDialog("El archivo " + textFieldNewFileName.getText()
									+ " ya existe ¿Quieres sobrescribirlo? (y/n)");
							if (res.equals("y")) {
						...
						JOptionPane.showMessageDialog(null, "Fusión realizada, puedes comprobarlo dentro del archivo "
								+ textFieldNewFileName.getText(), "ACTION BUTTON SEARCH",JOptionPane.INFORMATION_MESSAGE);
					} else {
						JOptionPane.showMessageDialog(null, "Los documentos no existen en el directorio que estamos, en la parte de datos te aparecen los documentos disponibles", "ACTION BUTTON SEARCH",JOptionPane.INFORMATION_MESSAGE);
					}

				} else {
					JOptionPane.showMessageDialog(null, "Algún apartado esta vacío, no se ha podido realizar la fusión.", "ACTION BUTTON SEARCH",JOptionPane.INFORMATION_MESSAGE);
	...
	/**
	 * Metodo al que llamamos para mostar el nombre del archivo en el text area.
	 * @param file pasamos un archivo en clase File que nos dara el nombre.
	 */
	...
	/**
	 * Metodo donde hace la comparación de la palabra facilitada y la compara con los 
	 * documentos dentro del directorio seleccionado.
	 */
	public void matchInFiles() {
		
		Scanner word;
		String[] checkWord = textMatch.getText().split(" ");
		...
		if(checkWord.length == 1) {
			...
			for (File file : orderFile) {
				...
				if (!textMatch.getText().equals("")) {
					try {
						word = new Scanner(new File(directory.getName() + File.separator + file.getName()));
						
						while (word.hasNext()) {
							// System.out.println(word.next()+" - "+textMatch.getText());
							if (word.next().equals(checkWord[0])) {
								countCom++;
							}
						}
					} catch (Exception ex) {
		...
		if(checkWord.length > 1) {
			JOptionPane.showMessageDialog(null, "Debes escribir una palabra para poder comparar", "ACTION BUTTON SEARCH",JOptionPane.INFORMATION_MESSAGE);
		}
		
		if (textMatch.getText().equals("")) {
			JOptionPane.showMessageDialog(null, "No se ha escrito palabra para buscar coincidencias.", "ACTION BUTTON SEARCH",JOptionPane.INFORMATION_MESSAGE);
		}
	}

	/**
	 * Metodo para ordenar los ficheros por lo que seleccionamos en la aplicación
	 * @param strAscDes le pasamos si es Ascendente o Descendente el orden.
	 * @param strNumSizDate le pasamos si va ordenar por Nombre, Tamaño o Fecha
	 */
	...
	/**
	 * Metodo para fusionar los archivos elgidos del directorio
	 * @param srtFile01 le pasamos el primer archivo en formato string
	 * @param srtFile02 le pasamos el segundo archivo en formato string
	 * @param srtNewFile le pasamos el nombre del nuevo archivo donde queremos volcar la información formato string
	 * @param s pasamos booleano para saber si quiere sobrescribir o no.
	 */
